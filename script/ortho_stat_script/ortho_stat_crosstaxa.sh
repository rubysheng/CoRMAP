...
function ortho_sum() {
  ...
  study_lst=`ls -1 ./input/*.fasta`
  for file in `ls -1 ./input/*.fasta`; do
    file=`basename $file`
    sp_name=${file%.fasta}
    bioawk -c fastx '{ print "HEADER|"$name }' ./input/$sp_name.fasta > ./analyze/orgin_${sp_name}.lst
    sed -i "s/HEADER/${sp_name}/g" ./analyze/orgin_${sp_name}.lst
    cat ./output/groups/groups.txt | awk -v species="$sp_name" '{for(i=1; i<=NF; i++) if ($i ~ species) print $i}' > ./analyze/clustered_${sp_name}.lst
    cat ./analyze/orgin_${sp_name}.lst ./analyze/clustered_${sp_name}.lst | sort | uniq -u > ./analyze/unclustered_${sp_name}.lst
  done
...
function ortho_stat() {
  ...
  ### step2 : count unclustered/clustered sequences per species
  study_all=`ls -1 input/*.fasta | wc -l`
  echo "Amount Of Species : "${study_all}
  echo
  # generate a output file to hold the starts
  echo  "Species_Acc" "Count_Totalpep" "Count_Clustered" "Count_Unclustered" > analyze/stat/cluster.stat  # HEADER
  i=1
  for file in `ls -1 ./input/*.fasta`; do
    file=`basename $file`
    sp_name=${file%.fasta}
    ...
    echo ${sp_name} ${SP_OR_CNT} ${SP_CL_CNT} ${SP_UN_CNT} >> ./analyze/stat/cluster.stat
    let i=i+1
  done
...
function ortho_perg_seq() {
  ...
  # shuf -n 2 ../allsp_groups.spname.txt > allsp_10g_groups.txt  # only test those all-species-included groups
  while IFS= read -r line; do
...
# step1
# in terminal run this step : counts of proteins for each species in each cluster group
###################################################################################################
# cat output/groups/groups.txt | \
# awk 'BEGIN {print "GROUP_NUM", "PRJNA?????1", "PRJNA?????2", "PRJNA?????3", \
#                   "PRJNA?????4", "PRJNA?????5", "PRJNA?????6", "PRJNA?????7", \
#                   "PRJNA?????8", "PRJNA?????9", "PRJNA????10"}
...
# print $1, sp1, sp2, sp3, sp4, sp5, sp6, sp7, sp8, sp9, sp10 }' \
#  > analyze/stat/groups.spname.counts.stat
###################################################################################################
